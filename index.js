...
async function searchYouTube(options) {
  ...
  if (!apiKey) return { results: [], error: "No se encuentra la variable YOUTUBE_API_KEY en Render." };
  ...
  try {
    ...
    if (!response.ok) {
      return { results: [], error: `Google API (${response.status}): ${data.error?.message || 'Error de autenticación o cuota'}` };
    }
    ...
    return { results: [], error: `Fallo de red al conectar con Google: ${err.message}` };
  }
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'video-search') {
    try {
      ...
      const errorMessage = `❌ **Error del bot:** \`${err.message}\``;
